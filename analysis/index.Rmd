...
Briefly, this software is designed to find connections between multiple genes, based on their gene expression. WGCNA provides a framework for constructing co-expression networks. In gene co-expression networks, each gene is represented by a node. In weighted gene networks, the strength of the connection is based on the similiarity of their gene expression (as measured by the Pearson correlation matrix). To read more about calculating gene co-expression networks, see the theory papers [here](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/). In addition to characterizing gene co-expression networks, there is also interest in integrating these networks with data from other traits, such as weight. 
