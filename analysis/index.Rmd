...
Briefly, the WGCNA software is designed to construct gene co-expression networks, that is, to find connections between multiple genes. These connections are based on the gene expression levels across samples. In these networks, each gene is represented by a node, and the strength of the connection between nodes (genes) is based on gene expression similarity. This similarity is usually measured by Pearson's correlation and then transformed. To read more about calculating gene co-expression networks, see the WGCNA theory papers [here](https://labs.genetics.ucla.edu/horvath/CoexpressionNetwork/). 

In addition to characterizing gene co-expression networks, there is also interest in integrating these networks with clinical data or other traits, such as weight. The goal of this process is identify potentially biologically meaningful groups of genes. By studying the meaningful groups of genes, as well as the key driver genes in each of these groups, one can develop genes of interest for these traits. These candidate gene lists could then be subjected to experimental validation causal genes or evaluated for their efficacy as biomarkers.
...
* To see the clinical data for each mouse, click [here](https://lauren-blake.github.io/Explore_WGCNA/Tutorial_1.html#add_the_clinical_data). 
